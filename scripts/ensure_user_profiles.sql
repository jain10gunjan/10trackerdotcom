-- Run this ONCE in Supabase → SQL Editor (single script, safe to re-run).
-- Creates user_profiles if missing, adds columns, fixes email PK, RLS, and reloads API schema.
--
-- DEADLOCK: Do NOT run at the same time as setup_platform_exams.sql or add_* scripts.
-- Close other SQL tabs, wait for each script to finish, then run the next.
-- If you already ran setup_platform_exams + add_dashboard_profile_columns, you can skip
-- duplicate target_exams line below (IF NOT EXISTS is safe).
...
-- 2) Legacy: table existed with `email` instead of `user_email`
...
EXCEPTION
  WHEN others THEN
    RAISE NOTICE 'PK on user_email not added (table may use a different primary key). App supports user_email or email column.';
END $$;
...
-- 6) Completion flags for existing rows
UPDATE public.user_profiles
...
-- 7) RLS — drop ALL policies (fixes "infinite recursion detected in policy")
...
-- 8) Refresh PostgREST schema cache (run once after ALL migration scripts finish)
-- NOTIFY pgrst, 'reload schema';
-- Prefer: Supabase Dashboard → Project Settings → API → Reload schema

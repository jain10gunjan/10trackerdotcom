...
import { buildHeatmapGrid } from '@/lib/activityHeatmap';
...
export function useHomeDashboard({ enabled = true } = {}) {
  ...
  const changeHeatmapRange = useCallback(
    (nextRange) => {
      ...
      setDashboard((prev) => {
        if (!prev?.activityEvents?.length) return prev;
        return {
          ...prev,
          heatmap: { range: nextRange, ...buildHeatmapGrid(prev.activityEvents, nextRange) },
        };
      });
    },

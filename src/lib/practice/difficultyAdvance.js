...
export function buildAdvancePrompt({ current, counts, scopeLabel = 'chapter' }) {
  ...
  if (skippedEmpty.length > 0) {
    ...
    return {
      kind: 'prompt',
      next,
      message: `${currentCap} complete — no ${skippedLabel} questions for this ${scope}. Move to ${nextCap}?`,
    };
  }
  ...
  return {
    kind: 'prompt',
    next,
    message: `${currentCap} complete! Move to ${nextCap}?`,
  };
}
...
/**
 * Show confirm prompt or celebration toast after finishing a difficulty level.
...
export function promptDifficultyAdvance({
  ...
  if (plan.kind === 'prompt' && confirmFn(plan.message)) {
    onAdvance?.(plan.next);
    return true;
  }

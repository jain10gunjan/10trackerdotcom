...
import { MapPin, Pencil, Coins, Trophy, BookOpen, BarChart3, Infinity } from 'lucide-react';
  ...
  return (
    <div className="flex items-start gap-3 py-2 border-b border-neutral-100 last:border-0">
      ...
      <div className="min-w-0 flex-1">
        ...
        {hint ? <p className="text-[10px] text-neutral-400 mt-0.5">{hint}</p> : null}
      </div>
    </div>
  );
}
  ...
  return (
    <aside className="space-y-4 lg:sticky lg:top-24 lg:self-start">
      <div className="rounded-2xl border border-neutral-200 bg-white shadow-sm overflow-hidden">
        <div className="p-5 border-b border-neutral-100">
          <div className="flex items-center gap-3">
            <div className="w-14 h-14 rounded-xl overflow-hidden bg-neutral-100 border border-neutral-200 flex items-center justify-center shrink-0">
              {avatarUrl ? (
                <Image
                  src={avatarUrl}
                  alt=""
            ...
            <div className="min-w-0 flex-1">
              <h2 className="text-base font-semibold text-neutral-900 truncate">{displayName}</h2>
              {user?.email ? (
          ...
          {profile?.bio ? (
            <p className="text-sm text-neutral-600 mt-3 leading-relaxed line-clamp-3">{profile.bio}</p>
          ) : null}

          {location ? (
            <p className="flex items-center gap-1.5 text-xs text-neutral-500 mt-3">
      ...
      <div className="rounded-2xl border border-neutral-200 bg-white shadow-sm px-4 py-3">
        <h3 className="text-[10px] font-semibold uppercase tracking-wide text-neutral-400 mb-1">
          At a glance
        </h3>
        <StatRow

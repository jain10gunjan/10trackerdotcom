...
const Pagetracker = () => {
  ...
  const reportQuestion = async (questionId, reason) => {
    ...
    try {
      const { error } = await supabase.from("reported_questions").insert({
        question_id: questionId, // Ensure this matches your table schema
        topic: pagetopic,
        user_id: user.email,
        reason: reason,
  ...
  // Calculate total questions for Sidebar
                  ...
                  {filteredQuestions.length > 0 ? (
                    filteredQuestions.map((question) => (
                      <QuestionCard
                        ...
                        onStartEditing={() =>
                          startEditingQuestion(question._id)
                        }
